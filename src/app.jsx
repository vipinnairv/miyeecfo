...
function PageTx({data,setData,toast,type}){
  ...
  /* Free-text search across every field you can see in a row, so you do not
     have to remember which column a thing lives in. Terms are ANDed: typing
     "swiggy upi" finds Swiggy paid by UPI without either being a filter.
     A bare number matches the amount, on the digits rather than the rendered
     string, so 1214 finds the same row whether it shows as 1,214 or 1214. */
  const terms=useMemo(()=>q.trim().toLowerCase().split(/\s+/).filter(Boolean),[q]);
  const matchQ=useCallback(t=>{
    if(!terms.length)return true;
    const hay=[t.desc,t.merchant,t.category,t.paymentMode,t.date,fmtMonth(t.month),
      String(t.amount),Math.round(t.amount).toLocaleString(CUR.locale)]
  ...
  const filtered=useMemo(()=>[...transactions.filter(t=>t.type===type
    ...
    &&matchQ(t))]
    .sort((a,b)=>new Date(b.date)-new Date(a.date)),[transactions,type,selM,selCat,selPM,selMerchant,matchQ]);
      ...
      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10,flexWrap:'wrap'}}>
        <div style={{position:'relative',flex:'1 1 260px',minWidth:200}}>
          <span style={{position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',
            pointerEvents:'none',display:'flex',color:'var(--n400)'}}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4">
              <circle cx="11" cy="11" r="7"/><path d="M20 20l-3.5-3.5"/></svg>
          </span>
          <input value={q} onChange={e=>setQ(e.target.value)}
            onKeyDown={e=>{if(e.key==='Escape')setQ('');}}
            placeholder="Search description, merchant, category, mode or amount"
            style={{width:'100%',padding:'7px 30px 7px 30px',fontSize:12}}/>
          {q&&<button type="button" onClick={()=>setQ('')} title="Clear (Esc)"
            style={{position:'absolute',right:6,top:'50%',transform:'translateY(-50%)',
              border:'none',background:'none',cursor:'pointer',color:'var(--n400)',
              fontSize:15,lineHeight:1,padding:'2px 5px'}}>×</button>}
        </div>
        {q.trim()&&(
          <span style={{fontSize:11.5,color:'var(--n500)',whiteSpace:'nowrap'}}>
            {filtered.length===0
              ? <span style={{color:'var(--r)',fontWeight:600}}>No match</span>
              : <><strong style={{fontFamily:'var(--m)'}}>{filtered.length}</strong> match{filtered.length===1?'':'es'}
                 {' · '}<strong style={{fontFamily:'var(--m)'}}>{fmtINR(total,true)}</strong></>}
          </span>
        )}
      </div>
      <div className="mbar">
